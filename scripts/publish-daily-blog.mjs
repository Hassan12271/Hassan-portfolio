...
const postsFile = path.join(__dirname, '../src/data/blog-posts.json');
const blogImages = ['/images/blog/blog1.jpg', '/images/blog/blog2.jpg', '/images/blog/blog3.jpg'];

const topicTemplates = [
  {
    title: 'Why Website Speed Matters for SEO and Conversions',
    excerpt: 'A fast website improves search rankings, user trust, and lead generation for service businesses.',
  ...
  },
  {
    ...
    excerpt: 'Identify outdated design, poor mobile experience, and weak conversion paths before they hurt growth.',
  ...
  },
  {
    ...
    excerpt: 'Understand when a React SPA is enough and when Next.js is the better production choice.',
    ...
    content: [
      ...
      'The best choice depends on project goals, timeline, and long-term maintenance.',
    ],
  },
  {
    title: 'How to Structure a High-Converting Landing Page',
    excerpt: 'Learn the section order and messaging flow that turns visitors into inquiries.',
  ...
  },
  {
    ...
    excerpt: 'Keep your WordPress site secure, fast, and reliable with a simple monthly routine.',
  ...
  },
  {
    ...
    excerpt: 'Showcase your work with context, tech stack, and outcomes that build client confidence.',
  ...
  },
  {
    ...
    excerpt: 'Build websites for phones first, then scale up to tablet and desktop layouts.',
  ...
  },
  {
    ...
    excerpt: 'Reduce friction and increase inquiries with better form design and placement.',
  ...
  },
  {
    ...
    excerpt: 'Metadata, sitemaps, canonical URLs, and structured data explained for production sites.',
  ...
  },
  {
    ...
    excerpt: 'Compare WordPress, React, and Next.js based on budget, speed, and future needs.',
...
function buildPostFromTemplate(template, date, imageIndex) {
...
async function generateWithOpenAI(template, date) {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You write concise, practical blog posts for Hassan Raza, a web and app developer in Karachi. Return valid JSON only.',
        },
        {
          role: 'user',
          content: `Write a blog post based on this topic seed:\nTitle: ${template.title}\nExcerpt: ${template.excerpt}\nTags: ${template.tags.join(', ')}\nReturn JSON with keys: title, excerpt, tags (array), content (array of 4 short paragraphs).`,
        },
      ],
  ...
  const baseSlug = slugify(parsed.title || template.title);

  return {
    slug: `${baseSlug}-${date}`,
    title: parsed.title || template.title,
    excerpt: parsed.excerpt || template.excerpt,
    date,
    author: 'Hassan Raza',
    designation: 'Web Developer',
    imgLink: blogImages[Number(date.replace(/-/g, '')) % blogImages.length],
    tags: Array.isArray(parsed.tags) ? parsed.tags : template.tags,
    content: Array.isArray(parsed.content) ? parsed.content : template.content,
  };
}

async function main() {
  const posts = readPosts();
  const today = process.env.BLOG_PUBLISH_DATE || getTodayDate();
  ...
  if (process.env.BLOG_AUTOMATION_MODE === 'openai') {
    newPost = await generateWithOpenAI(template, today);
  }

  if (!newPost) {
    newPost = buildPostFromTemplate(template, today, dayIndex);
  }
  ...
  const updatedPosts = [newPost, ...posts].slice(0, 120);
  writePosts(updatedPosts);

  console.log(`Published blog post: ${newPost.title}`);
